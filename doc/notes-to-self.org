...
a template contains one or more PDFs.
...
the mailinfo object is 1:1 with an uploaded agreement.
...
if we are in concatenate mode, we upload a docset to echosign, so the mailinfo object spans multiple PDFs.

if we are in non-concatenate mode, we upload an individual PDF to echosign, so we create one mailinfo per PDF.
